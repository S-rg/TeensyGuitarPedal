...
![Assembled pedal](imgs/GuitarPedalImg.JPG)
...
- A preamp circuit to boost the instrument-level input and buffer it for the ADC, and an output buffer to bring the DAC output back to an instrument level signal with low impedance which goes into an amp.
